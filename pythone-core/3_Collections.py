...
* Списки (list) -упорядоченная изменяемая колекция объектов (List в Java)
* Словари (dict) - неупорядоченное множество пар key/value (Map в Java)
* Кортеж (t uple) - упорядоченная не изменяемая коллекция объектов (Array в Java)
* Множество (set) - неупорядоченный набор неповторяющихся объектов (Set в Java)
...
empty_dict = {}
dict11 = dict()  # пустой словарь, по сути создание экземпляра класса
dict1 = {
    "key1": "value",
    "key2": "value",
    "key3": "value",
...
houses = {  # кортеж м.б. ключом т.к. не изменяемый. Это пример координат мест.
...
dict1.update(dict2)  # объединение словарей. Дублирующийся ключ во втором словаре перезаписывает значение из первого словаря (исключения здесь нет)
del dict1['key1']       # удаление ключа. Изменить ключ нельзя!!!
print('key2' in dict1)  # проверка наличия ключа
elem = dict1['key2']    # (!) получить значение по существующему ключу, если ключа нет, то ошибка
print(dict1.get('key1', 'Not in dict'))  # получить значение по ключу, если его нет, вернуть опциональное значение

print(dict1.setdefault("key777", 'value777'))  # получить значение по ключу, если его нет, добавить в словарь и вернуть его же
print(dict1.setdefault("key2", 'value999'))    #
print("setdefault: ", list(dict1.items()))
...
set1.symmetric_difference(set2)  # или set1 ^ set2 - XOR (исключаются общие элементы)
set1.issubset(set2)  # или set1 <= set2 проверка на подмножество (похож на знак принадлежности)

if {}:  # пустое множество интерпретируется как False, иначе True
    print('не пустое множество')
else:
    print('пустое множестово')
